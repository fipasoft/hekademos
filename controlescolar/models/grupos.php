...
class Grupos extends ActiveRecord{ 
	...
	public function alumnos(){
		...
		$alumnos = $alumnos->find_all_by_sql(
			"SELECT " .
				"alumnos.*, grupos.grado, grupos.letra, grupos.turno, " .
	...
	public function asignados(){
		...
		if( !is_array($asignados) ){
			...
			if( in_array('direccion',  $usr_grupos) ||
				...
				in_array('disciplina',  $usr_grupos)
				   )
	 		{
	 			$asignados[] = 'ALL';
	 		}else{
	...
	public function tutor(){
		...
		$tutores=$tutores->find_first("grupos_id='".$this->id."'");
		$profesor=new Profesores();
		$profesor=$profesor->find($tutores->profesores_id);
		return $profesor;
