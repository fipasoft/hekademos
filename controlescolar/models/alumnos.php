...
class Alumnos extends ActiveRecord{
	...
	public function totalamonestaciones(){
		$amonestaciones = new Amonestacion();
		return $amonestaciones->count_by_sql(
			"SELECT count(DISTINCT(amonestacion.id))
			FROM amonestacion
			INNER JOIN amonestados ON amonestacion.id = amonestados.amonestacion_id
			WHERE amonestados.alumnos_id = '".$this->id."' AND  amonestacion.aestado_id='2' "
		);
	}
