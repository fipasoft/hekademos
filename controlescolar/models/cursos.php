...
class Cursos extends ActiveRecord{
	...
	public function aula(){
		...
		$horario = $horarios->find_first("conditions: cursos_id = '" . $this->id . "'");

		$aula=new Aulas();
		$aula=$aula->find($horario->aulas_id);
		return $aula;
	}
	...
	public function alumnosconExtraordinario(){
		$alumnos=$this->alumnosInscritos();
		$this->calificacionesCapturadas();
		$cont=0;
		foreach($alumnos as $a){
			$clave=$this->oportunidadAlumno($a->id);
			if($clave=='sin-derecho' || $clave=='extraordinario'){
				$cont++;
			}
		}
		return $cont;
	...
	public function asignado($cache=true){
		...
		if($cache)
		$asignados = Session :: get_data('crs.asignados');
		else
		$asignados=null;
		...
		Session :: set_data('crs.asignados', $asignados);
	...
	public function calificacionesCaptura($accion){
		$aro=Session :: get_data('usr.login');
		$agenda = new Agenda();
		$materia = $this->materia();
		$ciclo =  $this->ciclo();
		$eventos = new Eventos();
	...
	public function horariostemporal(){
		$horarios = new Horariotemporal();
		$horarios = $horarios->find("conditions: cursos_id = '" . $this->id . "'",
									"order: dias_id");
		$hs = array();
		foreach($horarios as $h){
	...
	public function materia(){
		...
		if(trim($this->materias_id)!='')
		$materia = $Materias->find($this->materias_id);
		else
		$materia = $Materias->find($this->materia_id);
	...
	public function ccategoria(){
		$cc = new Ccategoria();
		$cc = $cc->find_first("cursos_id='".$this->id."'");
		return $cc;
	}
	
	public function hcategoria(){
		$cc = $this->ccategoria();
		$hc = new Hcategoria();
		$hc = $hc->find($cc->hcategoria_id);
		return $hc;
	}
	...
	public function faltas(){
		...
		$resumen = array();
		foreach($faltas as $fta){
			$resumen[$fta->alumnos_id] = $fta->faltas;
			}
