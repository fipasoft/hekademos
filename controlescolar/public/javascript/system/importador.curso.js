function init(){
		...
		if(frm_validar('frm_captura')){
			...
			$("upload_target").onload = function(){
				...
				Modalbox.show(respuesta,
						{title: "<h1>Importar alumnos a curso</h1>", width: 600, height: 700,
						afterLoad: function() {
							$('spinner1').hide();
									$("importar").onclick = function(){ 
										...
										if(valores!=""){
										new Ajax.Updater('contenido', $('kumbia_path').value + "importador/curso/", { 
											   method: 'post',
											   onLoading: function(){  $('spinner').show();  $('contenido').hide(); },
											   onComplete: function() { $('spinner').hide(); $('contenido').show()},
											   parameters: {did: $('did').value, alumnos: valores, articulos_id: $('articulos_id').value }
											});
										}else{
											try{
	...
	if(grupo){
		grupo.onchange = function() {
		 if(grupo.value!=""){
		 	new Ajax.Updater('div_cursos', './grupocursos/', {
			   method: 'post',
			   onLoading: function(){ $('spinner').show(); $('datos').hide(); },
			   onComplete: function() { $('spinner').hide(); div_sw('datos');},
			   parameters: {id: grupo.value}
			});
			}
		}
	}
