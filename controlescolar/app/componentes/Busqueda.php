<?php
// Hekademos, Creado el 19/09/2008
/**
 * Busqueda
 *
 * @package    Componentes
 ...
 */
 ...
 class Busqueda{
	...
	// carga en un arreglo los campos que integran la consulta
 		...
 		if($this->actualizar){
	 		...
	 		foreach($sesion[$controlador][$accion] as $var => $value){
				$op  = ($ops[$var]  != ''  ?  $ops[$var]   :  $this->operador);
	 			...
	 			if($value != '' && !in_array($var,$this->noCondicion)){
		 			$c .= ($c == '' ? '' : $op . ' ');
 	...
 	// establece el valor de un campo determinado
 	public function establecerCampo($nombre,$valor){
 	...
 	public function establecerCondicion($campo, $condicion){
 		...
 		$this->condiciones[$campo] = $condicion;
 	 	...
 	 	// quita campos de la condicion
 	public function quitarCondicion($campo){
 		$this->actualizar = true;
 		$this->noCondicion[$campo]=$campo;
 	}
