...
class XLSFinales extends Reporte{
    private $condicion;
    private $ciclo;
    private $grupo;
    private $nombre;

    public function XLSFinales($grupo_id = '', $alumnos = array()){
        $grupos = new Grupos();
        $ciclos = new Ciclos();
        ...
        $this->nombre = $nombre = 'Finales ' . $glabel . ' ' . $ciclo->numero . '.xls';
        $this->Reporte($nombre);
    ...
    public function hoja($alumnos){
        ...
        if(array_key_exists($nombre, $hojas)){
            ...
            $h = $this->agregar_hoja(substr($nombre,0,30), $rows, $cols);
            $h->cc_max = 6;
        }
    ...
    public function contenido(&$h, $alumnos){
        ...
        foreach($alumnos as $alumno){
            ...
            //$h->xls->write($h->rr, $h->cc, 'ASISTENCIAS %', $st['TH.BGGray']); $h->cc++;
            $h->xls->write($h->rr, $h->cc, 'ORDINARIO', $st['TH.BGGray']); $h->cc++;
            $h->xls->write($h->rr, $h->cc, 'EXTRAORDINARIO', $st['TH.BGGray']); $h->cc++;
            $h->nueva_linea();
            $n = 0;
            ...
            if(count($cursos) > 0){
                ...
                foreach($cursos as $curso){
                    ...
                    $porcentaje = ($asistencias['AST']/$nAsistencias)*100;
                    }*/
                    $ordinario = $c->calificacionAlumno($alumno->id, 'ORD');
                    $extra=$c->calificacionAlumno($alumno->id, 'EXT');

                    if($ordinario=="A")$ordinario="APROBADO";
                    elseif($ordinario=="NA")$ordinario="NO APROBADO";
                    ...
                    $h->xls->mergeCells($h->rr, $h->cc-3, $h->rr, $h->cc); $h->cc++;
                    //$h->xls->write($h->rr, $h->cc, $porcentaje, $st['TD.NormalCenter']); $h->cc++;
                    $h->xls->write($h->rr, $h->cc, $ordinario, $st['TD.NormalCenter']); $h->cc++;
                    $h->xls->write($h->rr, $h->cc, $extra, $st['TD.NormalCenter']); $h->cc++;
                    $h->nueva_linea();
                }
            }else{
    ...
    public function encabezado(&$h){
        ...
        $h->xls->mergeCells($h->rr, $h->cc, $h->rr, $h->cc + 6);
        $h->nueva_linea();
        $h->xls->write($h->rr, $h->cc, 'BOLETA DE CALIFICACIONES FINALES 2009A', $st['H4']);
        $h->xls->mergeCells($h->rr, $h->cc, $h->rr, $h->cc + 6);
        $h->nueva_linea();
        $h->nueva_linea();
    }
    ...
    public function pieDePagina(&$h){
        ...
        /*$h->xls->write(
                $h->rr,
                $h->cc,
                'ESTAS CALIFICACIONES SON FINALES.',
                $st['TD.NoBorder']
        );
        $h->xls->mergeCells($h->rr, $h->cc, $h->rr, $h->cc + 6);
        $h->nueva_linea();*/
        $h->xls->write(
                $h->rr,
                $h->cc,
                utf8_decode('Para Cualquier aclaración, favor de acudir a la Oficialía Mayor de esta Escuela. Teléfono 38114149 ext. 106'),
                $st['TD.NoBorderSmall']
        );
        $h->xls->mergeCells($h->rr, $h->cc, $h->rr, $h->cc + 6);
        $h->nueva_linea();
