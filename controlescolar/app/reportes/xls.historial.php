...
class XLSHistorial extends Reporte{
    ...
    public function contenido(&$h, $datos,$fecha){
        ...
        $n = 0;
            ...
            $h->xls->write($h->rr, $h->cc,utf8_decode($his->usuario), $st['TD' . $td . '.NormalCenter']); $h->cc++;
            $h->xls->write($h->rr, $h->cc, utf8_decode($his->controlador), $st['TD' . $td . '.NormalCenter']);$h->cc++;
            $h->xls->write($h->rr, $h->cc, utf8_decode($his->accion), $st['TD' . $td . '.NormalCenter']);$h->cc++;
            $h->xls->write($h->rr, $h->cc, utf8_decode($his->descripcion), $st['TD' . $td . '.NormalCenter']);$h->cc++;
