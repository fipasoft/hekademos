...
class XLSResumenReporte extends Reporte {
	private $condicion;
	public function XLSResumenReporte($id = '') {
	...
	public function contenido(& $h, $grupo,$alumnos) {
		...
		foreach($cursos as $curso){
			...
			$h->write_merge(utf8_decode($curso->materia), $h->rr, $h->cc, $h->rr, $h->cc+2, $st['TH.BGOrange']);
			$cc=$h->cc;

			$h->cc=$cc;
			$h->xls->write($h->rr+1, $h->cc, "AST %", $st['TH.BGYellow']);
			$h->cc++;
			$h->xls->write($h->rr+1, $h->cc, "ORD", $st['TH.BGYellow']);
			$h->cc++;
			$h->xls->write($h->rr+1, $h->cc, "EXT", $st['TH.BGYellow']);
			}
		}
