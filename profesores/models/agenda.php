 ...
 class Agenda extends ActiveRecord{
    ...
    public function eventoValido($clave,$ciclo_id,$aco_section,$aco,$aro){
        ...
        if($evento->id!=""){
            ...
            $myLog->log("hoy: ".$hoy.', inicio: '.$inicio.', fin: '.$fin, Logger::WARNING);

            if($inicio<=$hoy && $hoy<=$fin){
                if(strtoLower($aro)==strtoLower($rol->aro) || $rol->aro=="ALL" || in_array(strtoLower($rol->aro),$usr_grupos) || strtoLower($rol->aro)==strtoLower(Session :: get_data( 'prof.usr.codigo'))){
                    return true;
                }
            }
            }
