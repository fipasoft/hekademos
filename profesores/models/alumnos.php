...
class Alumnos extends ActiveRecord{
    ...
    public function asignadoprofesor(){
        $ciclo_id = Session :: get_data('prof.ciclo.id');
        $alucrs = new Alumnoscursos();
        //$alugrp = $alugrp->find_first("conditions: alumnos_id = '" . $this->id . "' ");
        $alucrs = $alucrs->find_all_by_sql(
                    " SELECT alumnoscursos.* " .
                    " FROM " .
                    " alumnoscursos " .
                    " INNER JOIN cursos ON alumnoscursos.cursos_id= cursos.id " .
                    " INNER JOIN grupos ON cursos.grupos_id=grupos.id ".
                    " WHERE grupos.ciclos_id='".$ciclo_id."' AND alumnoscursos.alumnos_id='" . $this->id . "' "
                    );


        if(count($alucrs)>0){
            $curso = new Cursos();
            $asignados = $curso->asignados();
            foreach($alucrs as $crs){
                if( in_array($crs->cursos_id, $asignados))
                {
                    return true;
                }
            }
    ...
    function cursos($ciclo_id){
            ...
            $cursos=$cursos->find_all_by_sql(
                    "SELECT cursos.*,materias.id AS materia_id,materias.nombre AS materia
                            FROM
                            alumnoscursos
                            INNER JOIN cursos ON alumnoscursos.cursos_id=cursos.id
    ...
    public function obtenerGrupoPorId($ciclo_id){
        $grupo = new Grupos();
        $grupo = $grupo->find_by_sql(
            "SELECT " .
                "grupos.id, " .
    ...
    public function estutor(){
        ...
        return (array_key_exists($this->grupos_id,$tutorias));
            ...
            $ciclo_id = Session :: get_data('prof.ciclo.id');
            $grupo=$this->obtenerGrupoPorId($ciclo_id);
            $tutorias=Session :: get_data('prof.usr.tutorias');
            return (array_key_exists($grupo->id,$tutorias));
