                ...
                if($curso->asignado()){
                    ...
                    if(
                        ($materia->duracion==7
                            &&
                        (
                        $agenda->eventoValido('CAS-CAP',$ciclo->id,$this->controlador,$this->accion,Session :: get_data('prof.usr.codigo'))
                            ||
                         $agenda->eventoValido('CAS-CAP-ESP',$ciclo->id,$this->controlador,$this->accion,Session :: get_data('prof.usr.codigo'))
                          )
                        )
                            ||
                       (
                       ($materia->duracion!=7
                            &&
                        (
                       $agenda->eventoValido('AST-CAP',$ciclo->id,$this->controlador,$this->accion,Session :: get_data('prof.usr.codigo'))
                            ||
                       $agenda->eventoValido('AST-CAP-ESP',$ciclo->id,$this->controlador,$this->accion,Session :: get_data('prof.usr.codigo'))
                        )
                        )
                       )
                        ){
                    ...
                    $this->alert = ' El curso no esta aprobado aun.';
                        ...
                        if(is_array($asistencias)){
                            ...
                            if($this->error == ''){
                                ...
                                $historial->ciclos_id= Session :: get_data('prof.ciclo.id');
                                $historial->usuario=trim(Session :: get_data('prof.usr.login').' '.Session :: get_data('prof.usr.codigo'));
                            ...
                            foreach($asistencias as $id => $valor){
                                ...
                                if($a->id != ''){
                                    if($a->cursos_id == $curso->id){
                                        ...
                                        $this->option = 'error';
                                        $this->error .= 'Id de curso inconsistente.';
                                        break;
                                    }
                                }else{
                                    $this->option = 'error';
                ...
                if($curso->id != ''){
                    ...
                    if(
                        ($materia->duracion==7
                            &&
                        (
                        $agenda->eventoValido('CAS-CAP',$this->ciclo->id,$this->controlador,$accion2,Session :: get_data('prof.usr.codigo'))
                            ||
                         $agenda->eventoValido('CAS-CAP-ESP',$this->ciclo->id,$this->controlador,$accion2,Session :: get_data('prof.usr.codigo'))
                          )
                        )
                            ||
                       (
                       ($materia->duracion!=7
                            &&
                        (
                       $agenda->eventoValido('AST-CAP',$this->ciclo->id,$this->controlador,$accion2,Session :: get_data('prof.usr.codigo'))
                            ||
                       $agenda->eventoValido('AST-CAP-ESP',$this->ciclo->id,$this->controlador,$accion2,Session :: get_data('prof.usr.codigo'))
                        )
                        )
                       )
                        ){
