...
class ApplicationControllerBase {
    ...
    public function before_filter($controlador, $accion, $evento) {
        ...
        {
            $usr->id = Session :: get_data('prof.usr.id');
            $usr->login = Session :: get_data('prof.usr.login');
            $usr->nombre = Session :: get_data('prof.usr.nombre');
        }
        ...
        if( (!$acl->acl_check('ALL',         'ALL',   'usuarios',  $usr->login) &&
            ...
            ||
            (
            !$acl->acl_check('ALL',         'ALL',   'usuarios',  'profesor') &&
            !$acl->acl_check($controlador,  'ALL',   'usuarios',  'profesor') &&
            !$acl->acl_check($controlador,  $accion, 'usuarios',  'profesor')
            )
        ){
